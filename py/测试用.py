...
class Spider(Spider):
    ...
    def playerContent(self, flag, id, vipFlags):
        # 使用 self.fetch 替代 requests，调用 Java 层网络栈
        # 这样可以避免 Python 层的线程/SSL 上下文问题
            ...
            try:
                ...
                if detail_matches:
                    ...
                    if mag_match:
                        ...
                        if parts:
                            # 强制转为字符串
                            clean_magnet = str('magnet:?' + '&'.join(parts))
                        else:
                            clean_magnet = str(full_magnet.split('&tr=')[0])
                ...
                return {'parse': 0, 'url': id, 'error': f'磁力异常', 'header': safe_headers}
    ...
    def get_video_list(self, url):
        ...
        for item in data('.movie').items():
            ...
            if not link or 'html' not in link: continue
    ...
    def getpq(self, path=''):
        ...
        try:
            ...
            # response 可能是 requests 对象或自定义对象，适配一下
            content = response.text if hasattr(response, 'text') else str(response)
            return pq(content)
        except Exception as e:
            return pq("<html></html>")
